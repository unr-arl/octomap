...
* The code is based on the implementation in OcTreeNode.h by K.M. Wurm and
* A. Hornung. See the file OcTreeNode.h for more information.
*/
...
public:
  ...
  public:
    ...
    Saliency()
        : value(0), value_buff(0), type(VOXEL_NORMAL), logodds(0), counter(0),
          timestamp(0), viewpoint(0), density(0) {
      for (int i = 0; i < R_BINS; i++)
        for (int j = 0; j < YAW_BINS; j++)
          hist[i][j] = 0;
    }
    Saliency(uint8_t sal)
        : value(0), value_buff(0), type(VOXEL_NORMAL), logodds(0), counter(0),
          timestamp(0), viewpoint(0), density(0) {
      value = sal;
      for (int i = 0; i < R_BINS; i++)
...
public:
  ...
  SaliencyOcTreeNode(const SaliencyOcTreeNode &rhs)
      : OcTreeNode(rhs), saliency(rhs.saliency) {}
  ...
  inline bool isSaliencySet() const { return (saliency.timestamp); }
...
public:
  ...
  */
  ...
  SaliencyOcTreeNode *averageNodeSaliency(float x, float y, float z,
                                          uint8_t sal) {
    OcTreeKey key;
    if (!this->coordToKeyChecked(point3d(x, y, z), key))
      return NULL;
  ...
  SaliencyOcTreeNode *integrateNodeSaliency(float x, float y, float z,
                                            uint8_t sal) {
    OcTreeKey key;
    if (!this->coordToKeyChecked(point3d(x, y, z), key))
      return NULL;
...
std::ostream &operator<<(std::ostream &out,
                         SaliencyOcTreeNode::Saliency const &s);
